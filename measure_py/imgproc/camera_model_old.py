...
class CameraModel():
    ...
    @classmethod
    def load_model_json(cls, camera_model: dict):
        # dummy camera instance
        camera = CameraModel('IMX477', False)
        print(f"Loading camera model from json object")

        # make sure camera info exist, else raise Errors
        try:
            camera.image_size = np.asarray(camera_model["image_size"])
            camera.is_fisheye = np.asarray(camera_model['is_fisheye'])
        except:
            raise Exception(f"No camera info found in json object")
        
        # modify dummy camera 
        try:
            camera.cm1 = np.asarray(camera_model['cm1'])
            camera.cd1 = np.asarray(camera_model['cd1'])
            camera.cm2 = np.asarray(camera_model['cm2'])
            camera.cd2 = np.asarray(camera_model['cd2'])
            camera.R = np.asarray(camera_model['R'])
            camera.T = np.asarray(camera_model['T'])
        except: # if encounter None object, then no assignment
            pass
        return camera
