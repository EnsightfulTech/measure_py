...
class StereoRectify():
    ...
    def rectify_image(self, left_img=None, right_img=None, sbs_img=None):
        """ 
        ...
        if left_img is not None and right_img is not None:
            ...
            # split
            left_img = sbs_img [:,          0:   self.width]
            right_img = sbs_img [:, self.width: 2*self.width]
        else:
        ...
        left_rect = cv2.remap(
            left_img, self.leftMapX, self.leftMapY, 
            interpolation=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT
        )
        right_rect = cv2.remap(
            right_img, self.rightMapX, self.rightMapY, 
            interpolation=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT
        )
